...
def loss_fn(params: torch.Tensor, n_pi: torch.Tensor, n_nu: torch.Tensor, beta_f: torch.Tensor, HFB_energies: torch.Tensor) -> torch.Tensor:
    """ Compute MAE loss between predicted IBM2 energies and target HFB energies """
    pred_energies = IBM2_PES(params, n_pi, n_nu, beta_f)
    return nn.L1Loss()(pred_energies, HFB_energies)
...
def calc_sae(pred_energies: torch.Tensor, target_energies: torch.Tensor) -> tuple[float, int]:
